...
@property uint64_t lastCPU,lastWall;
...
- (void)sample{
    ...
    _label.text=[NSString stringWithFormat:@"SB CPU %.1f%%%@",pct,pk];
...
- (NSString*)tableView:(UITableView*)t titleForHeaderInSection:(NSInteger)s{return @[@"基础",@"外观",@"CPU",@"提醒",@"自动注销"][s];}
- (NSInteger)tableView:(UITableView*)t numberOfRowsInSection:(NSInteger)s{return @[@3,@4,@5,@3,@3][s];}
- (UITableViewCell*)tableView:(UITableView*)t cellForRowAtIndexPath:(NSIndexPath*)p{
    ...
    if(p.section==0){title=@[@"启用悬浮窗",@"显示峰值",@"刷新间隔"][p.row];val=p.row==2?[NSString stringWithFormat:@"%.1f 秒",N(@"refresh",1)]:@"";}
    if(p.section==1){title=@[@"宽度",@"高度",@"透明度",@"圆角"][p.row];NSArray *appearanceValues=@[@(N(@"width",160)), @(N(@"height",48)), @(N(@"alpha",.82)), @(N(@"corner",10))];
        double v=[appearanceValues[p.row] doubleValue];val=[NSString stringWithFormat:@"%.2f",v];}
    if(p.section==2){title=@[@"低 CPU 阈值",@"高 CPU 阈值",@"峰值统计时间",@"刷新间隔",@"字体大小"][p.row];NSArray *cpuValues=@[@(N(@"low",30)), @(N(@"high",70)), @(N(@"peakWindow",60)), @(N(@"refresh",1)), @(N(@"font",13))];
        double v=[cpuValues[p.row] doubleValue];val=[NSString stringWithFormat:@"%.1f",v];}
    if(p.section==3){title=@[@"高 CPU 提醒",@"提醒阈值",@"持续时间"][p.row];}
    if(p.section==4){title=@[@"自动注销",@"注销阈值",@"持续时间"][p.row];}
    c.textLabel.text=title;
    BOOL sw=(p.section==0&&p.row<2)||(p.section==3&&p.row==0)||(p.section==4&&p.row==0);
    if(sw){
        UISwitch*s=[UISwitch new];BOOL on=NO;
        if(p.section==0)on=B(@[@"enabled",@"peak"][p.row],YES);
...
- (void)toggle:(UISwitch*)s{
    ...
    if(sec==0)WB(@[@"enabled",@"peak"][row],s.on);
...
- (void)tableView:(UITableView*)t didSelectRowAtIndexPath:(NSIndexPath*)p{
    ...
    NSArray*keys=@[@[@"enabled",@"peak",@"refresh"],@[@"width",@"height",@"alpha",@"corner"],@[@"low",@"high",@"peakWindow",@"refresh",@"font"],@[@"alert",@"alertThreshold",@"alertDuration"],@[@"autoRespring",@"respringThreshold",@"respringDuration"]];
    if((p.section==0&&p.row==2)){key=@"refresh";min=.2;max=10;cur=N(key,1);title=@"刷新间隔（秒）";}
    else if(p.section==1){key=keys[1][p.row];min=p.row==2?.2:1;max=p.row==0?300:p.row==1?150:p.row==2?1:40;NSArray *appearanceDefaults=@[@160, @48, @0.82, @10];
        cur=N(key,[appearanceDefaults[p.row] doubleValue]);title=@[@"宽度",@"高度",@"透明度",@"圆角"][p.row];}
    else if(p.section==2){key=keys[2][p.row];min=p.row<2?1:p.row==2?5:p.row==3?.2:9;max=p.row<2?100:p.row==2?600:p.row==3?10:24;NSArray *cpuDefaults=@[@30, @70, @60, @1, @13];
        cur=N(key,[cpuDefaults[p.row] doubleValue]);title=@[@"低 CPU 阈值 %",@"高 CPU 阈值 %",@"峰值统计秒数",@"刷新间隔秒",@"字体大小"][p.row];}
...
%ctor{
    ...
    B(@"enabled",YES);B(@"peak",YES);B(@"alert",YES);B(@"alertSound",YES);B(@"autoRespring",NO);
